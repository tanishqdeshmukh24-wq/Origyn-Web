...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function readForm() {
    ...
    return { ...data, price, categoryLabel: categoryLabels[data.category] || "Other", typeLabel: typeLabels[data.type] || "Other", image: imageLabels[data.category] || "OTHER", imageUrl: selectedImageUrl, rate, commission, sellerAmount: price - commission };
  }

  function updatePreview() {
    const data = readForm(); if (!data) return null; currentListing = data;
    const previewImage = $("#preview-image"), imageLabel = previewImage?.querySelector("span"), imageSmall = previewImage?.querySelector("small");
    if (data.imageUrl) { previewImage.style.backgroundImage = `url("${data.imageUrl}")`; previewImage.classList.add("has-product-image"); if (imageLabel) imageLabel.textContent = ""; if (imageSmall) imageSmall.textContent = ""; }
    else { previewImage.style.backgroundImage = ""; previewImage.classList.remove("has-product-image"); if (imageLabel) imageLabel.textContent = data.image; if (imageSmall) imageSmall.textContent = "ORIGYN"; }
    $("#preview-category").textContent = `${data.categoryLabel.toUpperCase()} · ${data.typeLabel.toUpperCase()}`; $("#preview-name").textContent = data.name; $("#preview-description").textContent = data.description; $("#preview-price").textContent = money(data.price); $("#preview-creator").textContent = data.creator; $("#preview-commission").textContent = `${Math.round(data.rate * 100)}% · ${money(data.commission)}`; $("#preview-payout").textContent = money(data.sellerAmount); preview.style.display = "block"; $("#form-message").textContent = "Preview ready. Review the listing and commission before publishing."; return data;
  }

  $("#product-image")?.addEventListener("change", event => {
    const file = event.target.files?.[0];
    if (!file) return;
    if (!file.type.startsWith("image/")) { $("#form-message").textContent = "Please choose a PNG, JPG or WEBP image."; return; }
    if (selectedImageUrl) URL.revokeObjectURL(selectedImageUrl);
    selectedImageUrl = URL.createObjectURL(file);
    const label = document.querySelector(".image-upload-box strong");
    if (label) label.textContent = file.name;
    const previewImage = $("#preview-image");
    previewImage.style.backgroundImage = `url("${selectedImageUrl}")`;
    ...
    else $("#form-message").textContent = "Image selected. Complete the listing to preview it.";
  });
