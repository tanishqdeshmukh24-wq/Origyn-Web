...
app.use(express.json({
  limit: '1mb',
  verify(req, _res, buf) {
    if (req.originalUrl.startsWith('/api/payments/webhooks/')) req.rawBody = Buffer.from(buf);
  }
}));
...
// Mount the more-specific seller-agreements path before the /api/seller prefix.
// This makes routing unambiguous because /api/seller-agreements is a prefix of
// /api/seller-agreements/* while /api/seller is also a broad seller prefix.
