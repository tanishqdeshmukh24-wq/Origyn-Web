...
(() => {
  function boot() {
    ...
    // Replace only the previous animation triggers. Application events/state remain untouched.
    ScrollTrigger.getAll().forEach(t => t.kill());
    gsap.killTweensOf("*");

    // Never leave interactive UI hidden.
    ...
    // HERO: cinematic sequence, not a blink.
    ...
    // DISCOVER heading gets its own explicit animation.
    ...
    // Product micro-interactions are visual only.
    ...
    // Story: preserve the existing characters, but make the entrance cinematic.
    ...
    // SUBMIT: explicitly animate every visible block; never hide the whole section.
